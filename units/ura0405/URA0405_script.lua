...
DroneCheckHeartBeat = function(self) 
    while not self:IsDead() do                        
        ...
        if self.Retaliation == true and self.MyAttacker != nil then
        ...
        if self.Retaliation == false and self.MyAttacker and self:GetDistanceToAttacker() <= 30 then
        ...
        elseif self.Retaliation == false and self:GetTargetEntity() then
...
ReceiveKills = function(self, unitKills, massKilled, killedNumber)
    if not self:IsDead() then
		CAirUnit.OnKilledUnit(self, unitKills, massKilled)
    end   
end,
...
#########################################################################################################
#																										#
#	Code copied from Unit.lua, Unit was unable to call this despite being of unit class type?	        #
#																										#
#                                                                                                       #
#########################################################################################################
